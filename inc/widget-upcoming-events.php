...
class Upcoming_Events extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			'class'			=>	'sis_upcoming_events',
			'description'	=>	__( 'A widget to display a list of upcoming events', 'sis' )
		);

		parent::__construct(
			'sis_upcoming_events',			//base id
			__( 'Upcoming Events', 'sis' ),	//title
			$widget_ops
		);
	}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title', 'sis' ); ?></label>
			<input type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" class="widefat" value="<?php echo esc_attr( $instance['title'] ); ?>">
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'number_events' ); ?>"><?php _e( 'Number of events to show', 'sis' ); ?></label>
		...
		<ul class="sis_event_entries">
			...
			?>
				<li class="sis_event_entry">
					<h4><a href="<?php the_permalink(); ?>" class="sis_event_title"><?php the_title(); ?></a> <span class="event_venue">at <?php echo $event_venue; ?></span></h4>
					<?php the_excerpt(); ?>
...
function sis_register_widget() {
	register_widget( 'Upcoming_Events' );
}
add_action( 'widgets_init', 'sis_register_widget' );